...
async def get_running_moves(
    ...
    return [{"id": move_id} for move_id in motion_manager.get_running_moves()]
...
async def goto(
    ...
    move_id, _ = motion_manager.create_move_task(
...
async def play_wake_up(
    ...
    move_id, _ = motion_manager.create_move_task(motion_manager.wake_up())
    return GotoStartedEvent(id=move_id)
...
async def play_goto_sleep(
    ...
    move_id, _ = motion_manager.create_move_task(motion_manager.goto_sleep())
    return GotoStartedEvent(id=move_id)
...
async def play_recorded_move_dataset(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=str(e))
    move_id, _ = motion_manager.create_move_task(motion_manager.play_move(move))
    return GotoStartedEvent(id=move_id)
...
async def cancel_goto(
    ...
    status = await motion_manager.stop_move_task(move_id)
...
async def get_goto_status(
    ...
    status = motion_manager.get_move_status(move_id)
    return GotoDoneEvent(id=move_id, status=status)
...
async def set_target(
    ...
    if motor_controller.is_move_running:
        ...
        motor_controller.logger.warning(
            "Ignoring set_target request: move already running."
        )
    ...
    if target.body_rotation is not None:
        motor_controller.set_target_body_yaw(
            target.body_rotation
        )  # API uses body_rotation, internal uses body_yaw
